...
- (void)configCustomNaviBar {
    ...
    _title = [[UILabel alloc] initWithFrame:CGRectMake(0,MARGIN_20, self.view.tz_width, 44)];
    _title.textColor = [UIColor whiteColor];
    _title.textAlignment = NSTextAlignmentCenter;
    
    [_naviBar addSubview:_title];
    
    _backButton = [[UIButton alloc] initWithFrame:CGRectMake(10, MARGIN_20, 44, 44)];
    [_backButton setImage:imageNamed(@"icon_navigation_back") forState:UIControlStateNormal];
    [_backButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [_backButton addTarget:self action:@selector(back) forControlEvents:UIControlEventTouchUpInside];
    if(_isCanSelect){
        _selectButton = [[UIButton alloc] initWithFrame:CGRectMake(self.view.tz_width - 54, MARGIN_20, 42, 42)];
        [_selectButton setImage:imageNamed(@"icon_login_upload_select_photo_white_line") forState:UIControlStateNormal];
        [_selectButton setImage:imageNamed(@"icon_login_upload_select_photo_white")  forState:UIControlStateSelected];
...
- (void)configBottomToolBar {
    _toolBar = [[UIView alloc] initWithFrame:CGRectMake(0, self.view.tz_height - 45, self.view.tz_width, 45)];
    _toolBar.backgroundColor = [UIColor colorFromHexString:@"ececec"];
    ...
    if(_isCanSelect){
        ...
        _okButton.frame = CGRectMake(self.view.tz_width - 80 - 12,MARGIN_5, 80,33);
        _okButton.layer.cornerRadius = 5;
        _okButton.titleLabel.font = FONT_OF_SIZE_14;
        [_okButton addTarget:self action:@selector(okButtonClick) forControlEvents:UIControlEventTouchUpInside];
        [_okButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        _okButton.backgroundColor = [UIColor blackColor];
        
        NSString *ok = _tzImagePickerVc.selectedModels.count <= 0?[NSString stringWithFormat:@"完成"]:[NSString stringWithFormat:@"完成(%zd)",_tzImagePickerVc.selectedModels.count];
...
- (void)configCollectionView {
    ...
    _collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0,NAVITETION_HEIGHT, self.view.tz_width , self.view.tz_height - NAVITETION_HEIGHT) collectionViewLayout:layout];
